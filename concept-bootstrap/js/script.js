(function () {
    ...
    var canvas = collection.insert("a", ".description").attr("href", function (d) {
        return "C:/Users/Shailesh/Desktop/concept/img/april2014_x_thumb.jpg";
    }).append("canvas");

    var pixelRatio = 1,
        storeRatio = 1;

    if (window.devicePixelRatio >= 2 && screen.availWidth >= 1280 // iPad can’t even handle it
        && canvas.node().getContext("2d").webkitBackingStorePixelRatio !== 2) { // Safari can’t even
        pixelRatio = 2;
        storeRatio = 2;
    }

    canvas.attr("height", height * storeRatio).style("height", height + "px");
    ...
    function scroll() {
        ...
        if (!canvas
            ...
            .each(enableFisheye)
            .empty()) {
            canvas = canvas.filter(function (d) {
                return !d.enabled;
            });
        }
    }
    ...
    function enableFisheye(d) {
        ...
        var x = fisheye().distortion(0).extent([0, width]);
        image.src = "img/"+d.slug+"/thumb.jpg";;
        //console.log("http://graphics8.nytimes.com/newsgraphics/2013/09/13/fashion-week-editors-picks/assets/thumbs-" + pixelRatio + "/" + d.slug + ".jpg");
        ...
        function mousedown() {
			...
			link.href = "javascript:popmodal('http://commondatastorage.googleapis.com/"+ d.slug +"/april2014_" + i + ".jpg',"+i+",'"+ d.slug +"');";
        }
